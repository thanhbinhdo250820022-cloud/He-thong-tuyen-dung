...
function renderRecruitmentTable(filtered){var data=filtered||recruitmentRequests;var c=document.getElementById('recruitmentTableContainer');if(!data.length){c.innerHTML='<p style="text-align:center;color:#999;padding:20px">Chưa có dữ liệu</p>';return}var h='<table id="recruitmentDataTable"><thead><tr><th>ID</th><th>Phòng ban</th><th>Vị trí tuyển dụng</th><th>Số lượng</th><th>Mô tả công việc</th><th>Yêu cầu ứng viên</th><th>Mức lương</th><th>Ngày yêu cầu</th><th>Trạng thái</th><th>File JD</th><th>Người thao tác</th><th>Thời gian thao tác</th><th>Thao tác</th></tr></thead><tbody>';data.forEach(function(r){var hired=getHiredCount(r.code);var remain=Math.max(0,r.quantity-hired);var status=remain>0?'<span class="badge badge-orange">Còn '+remain+'</span>':'<span class="badge badge-green">Đủ</span>';h+='<tr><td>'+r.code+'</td><td>'+r.department+'</td><td>'+r.position+'</td><td>'+r.quantity+'</td><td style="max-width:150px">'+r.jobDesc.substring(0,50)+'...</td><td>'+r.education+', '+na(r.experience)+'</td><td>'+na(r.salaryRange)+'</td><td>'+formatDate(r.needDate)+'</td><td>'+status+'</td><td>'+formatDate(r.deadline)+'</td><td>'+operatorInfo(r)+'</td><td>'+formatDateTime(r.timestamp)+'</td><td><button class="btn btn-edit btn-sm" onclick="startEditRecruitment(\''+r.code+'\')">Sửa</button> <button class="btn btn-delete btn-sm" onclick="if(confirm(\'Xóa?\'))deleteRecruitment(\''+r.code+'\')">Xóa</button> '+(remain>0?'<button class="btn btn-action btn-sm btn-upload-candidate" data-code="'+r.code+'">Tải lên thông tin ứng viên</button>':'')+'</td></tr>'});h+='</tbody></table>';c.innerHTML=h;c.querySelectorAll('.link-code[data-type="recruitment"]').forEach(function(lk){lk.addEventListener('click',function(){renderRecruitmentDetail(this.getAttribute('data-code'));showView('recruitmentDetailView')})});c.querySelectorAll('.btn-upload-candidate').forEach(function(b){b.addEventListener('click',function(){openCandidateFormFromRecruitment(this.getAttribute('data-code'))})})}
...
function renderCandidateTable(filtered){var data=filtered||candidates;var c=document.getElementById('candidateTableContainer');if(!data.length){c.innerHTML='<p style="text-align:center;color:#999;padding:20px">Chưa có dữ liệu</p>';return}var h='<table id="candidateDataTable"><thead><tr><th>ID ứng viên</th><th>Tên ứng viên</th><th>Ngày sinh</th><th>Giới tính</th><th>Số điện thoại</th><th>Email</th><th>Vị trí ứng tuyển</th><th>Nguồn tuyển dụng</th><th>CV</th><th>Trạng thái</th><th>Ghi chú</th><th>Người thao tác</th><th>Thời gian thao tác</th><th>Thao tác</th></tr></thead><tbody>';data.forEach(function(c2){var iv=interviews.find(function(x){return x.candidateCode===c2.code});var hasInterview=!!iv;var status=hasInterview?'<span class="badge badge-green">Đã đặt lịch</span>':'<span class="badge badge-orange">Chờ</span>';h+='<tr><td>'+c2.code+'</td><td>'+c2.fullName+'</td><td>'+formatDate(c2.dob)+'</td><td>'+c2.gender+'</td><td>'+c2.phone+'</td><td>-</td><td>'+c2.wish1+'</td><td>'+(c2.sources?c2.sources.join(', '):'')+'</td><td class="cv-cell">'+(c2.cvName?'<span class="link-code" data-cv="'+c2.code+'">'+c2.cvName+'</span>':'Không có')+'</td><td>'+status+'</td><td>'+c2.recruitCode+'</td><td>'+operatorInfo(c2)+'</td><td>'+formatDateTime(c2.timestamp)+'</td><td><button class="btn btn-edit btn-sm" onclick="startEditCandidate(\''+c2.code+'\')">Sửa</button> <button class="btn btn-delete btn-sm" onclick="if(confirm(\'Xóa?\'))deleteCandidate(\''+c2.code+'\')">Xóa</button> '+(!hasInterview?'<button class="btn btn-action btn-sm btn-schedule-from-cand" data-code="'+c2.code+'">Tạo lịch phỏng vấn</button>':'')+'</td></tr>'});h+='</tbody></table>';c.innerHTML=h;c.querySelectorAll('.link-code[data-cv]').forEach(function(lk){lk.addEventListener('click',function(){var cd=candidates.find(function(c){return c.code===lk.getAttribute('data-cv')});if(cd&&cd.cvURL)window.open(cd.cvURL,'_blank')})});c.querySelectorAll('.btn-schedule-from-cand').forEach(function(b){b.addEventListener('click',function(){openScheduleInterviewForm(this.getAttribute('data-code'))})})}
...
function renderCandidateTableForInterview(filtered){var data=filtered||candidates;var c=document.getElementById('interviewCandidateTableContainer');if(!data.length){c.innerHTML='<p style="text-align:center;color:#999;padding:20px">Chưa có ứng viên</p>';return}var h='<table><thead><tr><th>ID phỏng vấn</th><th>Tên ứng viên</th><th>Vị trí ứng tuyển</th><th>Phòng ban</th><th>Ngày phỏng vấn</th><th>Giờ phỏng vấn</th><th>Hình thức</th><th>Người phỏng vấn</th><th>Địa điểm / Link</th><th>Trạng thái</th><th>Ghi chú</th><th>Người thao tác</th><th>Thời gian thao tác</th><th>Thao tác</th></tr></thead><tbody>';data.forEach(function(c2){var iv=interviews.find(function(x){return x.candidateCode===c2.code});var rs=iv?interviewResults.find(function(r){return r.interviewCode===iv.code}):null;if(iv){h+='<tr><td>'+iv.code+'</td><td>'+c2.fullName+'</td><td>'+iv.position+'</td><td>'+c2.department+'</td><td>'+formatDate(iv.date)+'</td><td>'+iv.time+'</td><td>'+iv.tests.join(', ')+'</td><td>'+iv.interviewerName+'</td><td>'+iv.location+'</td><td><span class="badge badge-green">Đã đặt lịch</span></td><td>-</td><td>'+operatorInfo(iv)+'</td><td>'+formatDateTime(iv.timestamp)+'</td><td><button class="btn btn-edit btn-sm" onclick="alert(\'Chức năng sửa lịch PV\')">Sửa</button> <button class="btn btn-delete btn-sm" onclick="alert(\'Chức năng xóa lịch PV\')">Xóa</button> '+(!rs?'<button class="btn btn-warning btn-sm btn-evaluate-from-list" data-ivcode="'+iv.code+'">Điền kết quả phỏng vấn</button>':'<span class="badge badge-green">Đã đánh giá</span>')+'</td></tr>'}else{h+='<tr><td>-</td><td>'+c2.fullName+'</td><td>'+c2.wish1+'</td><td>'+c2.department+'</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td><span class="badge badge-orange">Chưa đặt lịch</span></td><td>-</td><td>'+operatorInfo(c2)+'</td><td>'+formatDateTime(c2.timestamp)+'</td><td><button class="btn btn-primary btn-sm btn-schedule-iv" data-code="'+c2.code+'">Đặt lịch</button></td></tr>'}});h+='</tbody></table>';c.innerHTML=h;c.querySelectorAll('.btn-schedule-iv').forEach(function(b){b.addEventListener('click',function(){openScheduleInterviewForm(this.getAttribute('data-code'))})});c.querySelectorAll('.btn-evaluate-from-list').forEach(function(b){b.addEventListener('click',function(){showEvaluationForm(this.getAttribute('data-ivcode'));showView('interviewResultFormView')})})}
...
function renderResultInterviewTable(filtered){var data=filtered||interviews;var c=document.getElementById('resultInterviewTableContainer');document.getElementById('resultFormContainer').style.display='none';if(!data.length){c.innerHTML='<p style="text-align:center;color:#999;padding:20px">Chưa có dữ liệu</p>';return}var h='<table><thead><tr><th>ID kết quả</th><th>Tên ứng viên</th><th>Vị trí ứng tuyển</th><th>Ngày phỏng vấn</th><th>Người phỏng vấn</th><th>Điểm đánh giá</th><th>Đánh giá chuyên môn</th><th>Đánh giá thái độ</th><th>Đánh giá tổng thể</th><th>Kết quả</th><th>Đề xuất lương</th><th>Ghi chú</th><th>Người thao tác</th><th>Thời gian thao tác</th><th>Thao tác</th></tr></thead><tbody>';data.forEach(function(iv){var cd=candidates.find(function(c){return c.code===iv.candidateCode});var rs=interviewResults.find(function(r){return r.interviewCode===iv.code});if(rs){var techTotal=0;rs.techScores.forEach(function(s){techTotal+=s});var softTotal=0;rs.softScores.forEach(function(s){softTotal+=s});h+='<tr><td>'+iv.code+'</td><td>'+(cd?cd.fullName:'')+'</td><td>'+iv.position+'</td><td>'+formatDate(iv.date)+'</td><td>'+iv.interviewerName+'</td><td>'+rs.totalScore+'/50</td><td>'+techTotal+'/25</td><td>'+softTotal+'/25</td><td>'+rs.suitability+'</td><td><span class="badge '+(rs.conclusion==="Đề xuất tuyển"?"badge-green":(rs.conclusion==="Dự bị"?"badge-orange":"badge-red"))+'">'+rs.conclusion+'</span></td><td>'+(cd&&cd.offer?cd.offer.officialSalary:'-')+'</td><td>-</td><td>'+operatorInfo(rs)+'</td><td>'+formatDateTime(rs.timestamp)+'</td><td><button class="btn btn-info btn-sm btn-view-result" data-ivcode="'+iv.code+'">Sửa</button> '+(rs.conclusion==="Đề xuất tuyển"&&!onboardingRecords.find(function(o){return o.candidateCode===rs.candidateCode})?'<button class="btn btn-success btn-sm btn-offer" data-ivcode="'+iv.code+'">Chuyển sang nhân viên mới</button>':'')+'</td></tr>'}else{h+='<tr><td>'+iv.code+'</td><td>'+(cd?cd.fullName:'')+'</td><td>'+iv.position+'</td><td>'+formatDate(iv.date)+'</td><td>'+iv.interviewerName+'</td><td>-</td><td>-</td><td>-</td><td>-</td><td><span class="badge badge-orange">Chưa</span></td><td>-</td><td>-</td><td>'+operatorInfo(iv)+'</td><td>'+formatDateTime(iv.timestamp)+'</td><td><button class="btn btn-warning btn-sm btn-evaluate" data-ivcode="'+iv.code+'">Điền kết quả phỏng vấn</button></td></tr>'}});h+='</tbody></table>';c.innerHTML=h;c.querySelectorAll('.btn-evaluate').forEach(function(b){b.addEventListener('click',function(){showEvaluationForm(this.getAttribute('data-ivcode'))})});c.querySelectorAll('.btn-view-result').forEach(function(b){b.addEventListener('click',function(){showResultDetailView(this.getAttribute('data-ivcode'))})});c.querySelectorAll('.btn-offer').forEach(function(b){b.addEventListener('click',function(){showOfferForm(this.getAttribute('data-ivcode'))})})}
...
function renderOnboardingList(filtered){checkOnboardingExpired();var data=filtered||interviewResults.filter(function(r){return r.conclusion==='Đề xuất tuyển'});var c=document.getElementById('onboardingListContainer');document.getElementById('onboardingFormContent').style.display='none';if(!data.length){c.innerHTML='<p style="text-align:center;color:#999;padding:20px">Chưa có</p>';return}var h='<table><thead><tr><th>ID nhân viên</th><th>Tên nhân viên</th><th>Vị trí</th><th>Phòng ban</th><th>Ngày nhận việc</th><th>Mức lương</th><th>Loại hợp đồng</th><th>Người quản lý</th><th>Trạng thái</th><th>Ghi chú</th><th>Người thao tác</th><th>Thời gian thao tác</th><th>Thao tác</th></tr></thead><tbody>';data.forEach(function(r){var cd=candidates.find(function(c){return c.code===r.candidateCode});var ob=onboardingRecords.find(function(o){return o.candidateCode===r.candidateCode});var iv=interviews.find(function(x){return x.code===r.interviewCode});var st=r.expiredStatus||(ob?'Đã nhận việc':'Chờ xác nhận');var bc=ob?'badge-green':(r.expiredStatus?'badge-red':'badge-orange');h+='<tr><td>'+(ob?ob.newEmployeeId:'-')+'</td><td>'+(cd?cd.fullName:'')+'</td><td>'+r.position+'</td><td>'+(cd?cd.department:'')+'</td><td>'+(ob?formatDate(ob.startDate):'-')+'</td><td>'+(cd&&cd.offer?cd.offer.officialSalary:'-')+'</td><td>-</td><td>'+(iv?iv.interviewerName:'-')+'</td><td><span class="badge '+bc+'">'+st+'</span></td><td>-</td><td>'+(ob?operatorInfo(ob):operatorInfo(r))+'</td><td>'+formatDateTime(ob?ob.timestamp:r.timestamp)+'</td><td>';if(!ob&&!r.expiredStatus){h+='<button class="btn btn-edit btn-sm" onclick="alert(\'Sửa\')">Sửa</button> <button class="btn btn-delete btn-sm" onclick="alert(\'Xóa\')">Xóa</button> <button class="btn btn-success btn-sm btn-do-onboard" data-candcode="'+r.candidateCode+'">Xác nhận nhận việc</button>'}else if(ob){h+='<span class="badge badge-green">Đã xác nhận</span>'}h+='</td></tr>'});h+='</tbody></table>';c.innerHTML=h;c.querySelectorAll('.btn-do-onboard').forEach(function(b){b.addEventListener('click',function(){showOnboardingForm(this.getAttribute('data-candcode'))})})}
