// PHIEN BAN CAP NHAT - FULL FEATURES V3
...
function handleApi(req, res) {
 ...
 // === SEARCH BY DATE RANGE (Updated: fromDate / toDate) ===
 if (pathname === "/api/recruitment/search" && req.method === "GET") {
 var fromDate = query.fromDate || query.date || '';
 var toDate = query.toDate || '';
 // backward compat: if only date param, use as single date
 if (!query.fromDate && query.date) { fromDate = query.date; toDate = query.date; }
 var filtered = database.recruitmentRequests.filter(function(r) {
 if (!fromDate && !toDate) return true;
 return isDateInRange(r.timestamp, fromDate, toDate);
 });
 res.writeHead(200, { "Content-Type": "application/json" });
 ...
 if (!fromDate && !toDate) return true;
 return isDateInRange(c.timestamp, fromDate, toDate);
 });
 res.writeHead(200, { "Content-Type": "application/json" });
 res.end(JSON.stringify(filtered));
 return true;
 ...
 if (!fromDate && !toDate) return true;
 // Tìm theo Ngày phỏng vấn (iv.date)
 return isDateInRange(iv.date, fromDate, toDate);
 });
 res.writeHead(200, { "Content-Type": "application/json" });
 res.end(JSON.stringify(filtered));
 return true;
 ...
 if (!fromDate && !toDate) return true;
 // Tìm theo Ngày cập nhật kết quả (timestamp)
 return isDateInRange(r.timestamp, fromDate, toDate);
 });
 res.writeHead(200, { "Content-Type": "application/json" });
 res.end(JSON.stringify(filtered));
 ...
 if (!fromDate && !toDate) return true;
 // Tìm theo Ngày nhận việc (startDate)
 return isDateInRange(e.startDate, fromDate, toDate);
 });
 res.writeHead(200, { "Content-Type": "application/json" });
 res.end(JSON.stringify(filtered));
 return true;
 ...
 // === GET INTERVIEW TESTS FOR A CANDIDATE ===
 ...
 formatDateTimeVN(iv.lastEditTimestamp || iv.timestamp)];
 });
 ...
 formatDateTimeVN(e.lastEditTimestamp || e.timestamp)];
 });
...
<!-- ===== INTERVIEW VIEW (UPDATED: show table directly with Evaluate Candidate button) ===== -->
...
<button class="btn btn-success" id="btnAddInterview">+ Đặt lịch mới</button>
</div>
...
<!-- ===== RESULT VIEW (UPDATED: show table directly with Inform button) ===== -->
...
<!-- Evaluate Candidate Form View -->
<div id="evaluateCandidateFormView" class="view">
<button class="btn btn-back" id="btnBackFromEvaluateCandidate">← Quay lại</button>
<h2>Đánh giá ứng viên</h2>
<div id="evaluateCandidateFormContent"></div>
</div>

<!-- Inform Result Form View -->
<div id="informResultFormView" class="view">
<button class="btn btn-back" id="btnBackFromInformResult">← Quay lại</button>
<h2>Thông báo kết quả cho ứng viên</h2>
<div id="informResultFormContent"></div>
</div>
...
// ===== NEW: INTERVIEW TABLE (shown directly on interviewView) =====
function renderInterviewMainTable(filtered){var data=filtered||interviews;var c=document.getElementById('interviewMainTableContainer');if(!data.length){c.innerHTML='<p style="text-align:center;color:#999;padding:20px">Chưa có lịch phỏng vấn</p>';return}var h='<table id="interviewMainDataTable"><thead><tr><th>STT</th><th>Mã lịch</th><th>Mã ứng viên</th><th>Họ tên</th><th>Vị trí</th><th>Ngày PV</th><th>Giờ PV</th><th>Hình thức</th><th>Người PV</th><th>Địa điểm</th><th>Trạng thái</th><th>Lần sửa</th><th>Người thao tác</th><th>Thời gian thao tác</th><th>Thao tác</th></tr></thead><tbody>';data.forEach(function(iv,i){var cd=candidates.find(function(x){return x.code===iv.candidateCode});var editCount=getEditCount(iv);h+='<tr><td>'+(i+1)+'</td><td><a href="/api/pdf/interview/'+encodeURIComponent(iv.code)+'" target="_blank" class="link-code">'+iv.code+'</a></td><td>'+iv.candidateCode+'</td><td>'+(cd?cd.fullName:'')+'</td><td>'+iv.position+'</td><td>'+formatDate(iv.date)+'</td><td>'+(iv.time||'')+'</td><td>'+(iv.interviewType||'Offline')+'</td><td>'+(iv.interviewerName||'')+'</td><td>'+(iv.location||'')+'</td><td><span class="badge badge-blue">'+(iv.status||'Đã lên lịch')+'</span></td><td>'+getEditCountBadge(iv)+'</td><td>'+operatorInfo(iv)+'</td><td>'+formatDateTime(iv.lastEditTimestamp||iv.timestamp)+'</td>';
// Evaluate Candidate button
var rs=interviewResults.find(function(r){return r.interviewCode===iv.code});
if(!rs){h+='<td><button class="btn btn-warning btn-sm btn-evaluate-cand" data-ivcode="'+iv.code+'">Evaluate Candidate</button></td>'}else{h+='<td><span class="badge badge-green">Đã đánh giá</span></td>'}
h+='</tr>'});h+='</tbody></table>';c.innerHTML=h;
c.querySelectorAll('.btn-evaluate-cand').forEach(function(b){b.addEventListener('click',function(){showEvaluationForm(this.getAttribute('data-ivcode'))})})}

// ===== NEW: RESULT TABLE (shown directly on interviewResultFormView) =====
function renderResultMainTable(filtered){var data=filtered||interviewResults;var c=document.getElementById('resultMainTableContainer');if(!data.length){c.innerHTML='<p style="text-align:center;color:#999;padding:20px">Chưa có kết quả</p>';return}var h='<table id="resultMainDataTable"><thead><tr><th>STT</th><th>Mã kết quả</th><th>Mã ứng viên</th><th>Họ tên</th><th>Vị trí</th><th>Người PV</th><th>Điểm đánh giá</th><th>Kết quả</th><th>Lương đề xuất</th><th>Ngày cập nhật</th><th>Ghi chú</th><th>Lần sửa</th><th>Người thao tác</th><th>Thời gian thao tác</th><th>Thao tác</th></tr></thead><tbody>';data.forEach(function(r,i){var cd=candidates.find(function(x){return x.code===r.candidateCode});h+='<tr><td>'+(i+1)+'</td><td>'+(r.code||'')+'</td><td>'+(r.candidateCode||'')+'</td><td>'+(cd?cd.fullName:'')+'</td><td>'+(r.position||'')+'</td><td>'+(r.employeeName||'')+'</td><td>'+(r.totalScore!==undefined?r.totalScore+'/50':'')+'</td><td>'+(r.conclusion||'')+'</td><td>'+(r.proposedSalary||r.salaryRange||'')+'</td><td>'+formatDateTime(r.timestamp)+'</td><td>'+(r.notes||r.strengths||'')+'</td><td>'+getEditCountBadge(r)+'</td><td>'+operatorInfo(r)+'</td><td>'+formatDateTime(r.lastEditTimestamp||r.timestamp)+'</td>';
// Inform button
h+='<td><button class="btn btn-info btn-sm btn-inform-result" data-rcode="'+(r.code||'')+'">Inform</button></td>';
h+='</tr>'});h+='</tbody></table>';c.innerHTML=h;
c.querySelectorAll('.btn-inform-result').forEach(function(b){b.addEventListener('click',function(){showInformForm(this.getAttribute('data-rcode'))})})}

// ===== INFORM FORM =====
function showInformForm(rCode){var r=interviewResults.find(function(x){return x.code===rCode});if(!r)return;var cd=candidates.find(function(x){return x.code===r.candidateCode});var ct=document.getElementById('informResultFormContent');var h='<div class="form-section"><h3>Thông báo kết quả cho: '+(cd?cd.fullName:'')+'</h3>';h+='<div class="info-row"><span class="info-label">Mã kết quả:</span><span class="info-value">'+r.code+'</span></div>';h+='<div class="info-row"><span class="info-label">Kết quả:</span><span class="info-value"><strong>'+r.conclusion+'</strong></span></div>';h+='<div class="info-row"><span class="info-label">Điểm:</span><span class="info-value">'+(r.totalScore!==undefined?r.totalScore+'/50':'N/A')+'</span></div>';
if(r.conclusion==='Đề xuất tuyển'){h+='<p style="color:#2e7d32;font-weight:700;margin:15px 0">Ứng viên ĐẠT - Có thể tiến hành thông báo trúng tuyển và chuyển sang nhận việc.</p>';h+='<button class="btn btn-success" id="btnProceedOffer" data-ivcode="'+(r.interviewCode||'')+'">Chuyển sang Offer</button>'}else if(r.conclusion==='Dự bị'){h+='<p style="color:#ef6c00;font-weight:700;margin:15px 0">Ứng viên DỰ BỊ - Thông báo cho ứng viên chờ kết quả.</p>'}else{h+='<p style="color:#c62828;font-weight:700;margin:15px 0">Ứng viên KHÔNG ĐẠT - Thông báo kết quả không trúng tuyển.</p>'}
h+='</div>';ct.innerHTML=h;showView('informResultFormView');
var btnOffer=document.getElementById('btnProceedOffer');if(btnOffer){btnOffer.addEventListener('click',function(){showOfferForm(this.getAttribute('data-ivcode'))})}}
...
// UPDATED: Interview view now shows interview table directly
document.getElementById('btnGoInterview').addEventListener('click',function(){renderInterviewMainTable();showView('interviewView')});
// UPDATED: Result view now shows result table directly
document.getElementById('btnGoResult').addEventListener('click',function(){renderResultMainTable();showView('interviewResultFormView')});
...
document.getElementById('btnBackFromScheduleInterview').addEventListener('click',function(){renderInterviewMainTable();goBack('interviewView')});
document.getElementById('btnBackFromInterviewResult').addEventListener('click',function(){goBack('mainView')});
document.getElementById('btnBackFromResultDetail').addEventListener('click',function(){renderResultMainTable();goBack('interviewResultFormView')});
document.getElementById('btnBackFromProposedExcel').addEventListener('click',function(){renderResultMainTable();goBack('interviewResultFormView')});
document.getElementById('btnBackFromOffer').addEventListener('click',function(){renderResultMainTable();goBack('interviewResultFormView')});
...
document.getElementById('btnBackFromEvaluateCandidate').addEventListener('click',function(){renderInterviewMainTable();goBack('interviewView')});
document.getElementById('btnBackFromInformResult').addEventListener('click',function(){renderResultMainTable();goBack('interviewResultFormView')});
...
// NEW: Add Interview button
document.getElementById('btnAddInterview').addEventListener('click',function(){
 // Show candidate list for selection
 var sel=document.getElementById('ivCandidateSelect');sel.innerHTML='<option value="">-- Chọn ứng viên --</option>';candidates.forEach(function(cd){var iv=interviews.find(function(x){return x.candidateCode===cd.code});if(!iv){sel.innerHTML+='<option value="'+cd.code+'">'+cd.code+' - '+cd.fullName+'</option>'}});
 var ps=document.getElementById('ivPosition');ps.innerHTML='<option value="">-- Chọn --</option>';recruitmentRequests.forEach(function(r){if(getRemainingQuantity(r)>0){var o=document.createElement('option');o.value=r.position+' ('+r.code+')';o.textContent=r.position+' ('+r.code+') [Còn '+getRemainingQuantity(r)+']';ps.appendChild(o)}});
 document.getElementById('ivInterviewerCode').value='';document.getElementById('ivInterviewerInfo').style.display='none';document.getElementById('ivDate').value='';document.getElementById('ivTime').value='';document.getElementById('ivLocation').selectedIndex=0;document.getElementById('ivType').selectedIndex=0;document.querySelectorAll('input[name="ivTest"]').forEach(function(cb){cb.checked=false});showView('scheduleInterviewFormView')});

// Interviewer code lookup
...
// Interview search: by Ngày phỏng vấn (iv.date)
document.getElementById('btnSearchInterview').addEventListener('click',function(){var f=document.getElementById('interviewSearchFrom').value,t=document.getElementById('interviewSearchTo').value;var txt=(document.getElementById('interviewSearchText').value||'').trim().toLowerCase();renderInterviewMainTable(interviews.filter(function(iv){var d=iv.date||'';var matchDate=(!f||d>=f)&&(!t||d<=t);var cd=candidates.find(function(c){return c.code===iv.candidateCode});var matchText=!txt||(cd&&cd.fullName.toLowerCase().includes(txt))||iv.candidateCode.toLowerCase().includes(txt)||iv.code.toLowerCase().includes(txt);return matchDate&&matchText}))});

// Result search: by Ngày cập nhật kết quả (timestamp)
document.getElementById('btnSearchResult').addEventListener('click',function(){var f=document.getElementById('resultSearchFrom').value,t=document.getElementById('resultSearchTo').value;var txt=(document.getElementById('resultSearchText').value||'').trim().toLowerCase();renderResultMainTable(interviewResults.filter(function(r){var ts=(r.timestamp||'').substring(0,10);var matchDate=(!f||ts>=f)&&(!t||ts<=t);var cd=candidates.find(function(c){return c.code===r.candidateCode});var matchText=!txt||(cd&&cd.fullName.toLowerCase().includes(txt))||r.candidateCode.toLowerCase().includes(txt)||(r.code&&r.code.toLowerCase().includes(txt));return matchDate&&matchText}))});

// Onboarding search: by Ngày nhận việc (startDate)
document.getElementById('btnSearchOnboarding').addEventListener('click',function(){var f=document.getElementById('onboardSearchFrom').value,t=document.getElementById('onboardSearchTo').value;var txt=(document.getElementById('onboardSearchText').value||'').trim().toLowerCase();renderOnboardingList(onboardingRecords.filter(function(e){var d=e.startDate||'';var matchDate=(!f||d>=f)&&(!t||d<=t);var matchText=!txt||(e.candidateName&&e.candidateName.toLowerCase().includes(txt))||(e.employeeCode&&e.employeeCode.includes(txt));return matchDate&&matchText}))});
