// PHIÊN BẢN CẬP NHẬT - SỬA LẠI CÁC BẢNG HIỂN THỊ
...
function loadFromJsonBin() {
 ...
 if (!database.counters) database.counters = { recruitmentRequestCounter: 1, candidateCounter: 1, interviewFormCounter: 1, resultCounter: 1, employeeCounter: 1 };
 if (!database.counters.resultCounter) database.counters.resultCounter = 1;
 if (!database.counters.employeeCounter) database.counters.employeeCounter = 1;
 isDataLoaded = true;
...
<div class="form-row"><div class="form-group"><label>Mã ứng viên *</label><input type="text" id="ivCandCode" placeholder="VD: C00001"></div>
...
<div class="form-section">
<div class="form-row"><div class="form-group"><label>Mã ứng viên *</label><input type="text" id="resCandCode" placeholder="VD: C00001"></div>
...
const scriptContent = `
<script>
var recruitmentRequestCounter=1,interviewFormCounter=1,candidateCounter=1,resultCounter=1,employeeCounter=1;
...
function loadDataFromServer(){return fetch('/api/data').then(function(response){if(!response.ok)throw new Error('HTTP '+response.status);return response.json()}).then(function(data){if(data.recruitmentRequests)recruitmentRequests=data.recruitmentRequests;if(data.candidates)candidates=data.candidates;if(data.interviews)interviews=data.interviews;if(data.interviewResults)interviewResults=data.interviewResults;if(data.onboardingRecords)onboardingRecords=data.onboardingRecords;if(data.history)actionHistory=data.history;if(data.counters){recruitmentRequestCounter=data.counters.recruitmentRequestCounter||1;candidateCounter=data.counters.candidateCounter||1;interviewFormCounter=data.counters.interviewFormCounter||1;resultCounter=data.counters.resultCounter||1;employeeCounter=data.counters.employeeCounter||1}console.log('Client: Da tai du lieu')}).catch(function(err){console.error('Client: Loi tai du lieu:',err)})}
...
function generateResultCode(){return'R'+String(resultCounter++).padStart(5,'0')}
function generateEmployeeCode(){return'NV'+String(employeeCounter++).padStart(5,'0')}
...
var status=r.status||'Đang tuyển';
...
h+='<td><select class="status-select rec-status-change" data-code="'+r.code+'"><option value="Đang tuyển"'+(status==='Đang tuyển'?' selected':'')+'>Đang tuyển</option><option value="Đã tuyển đủ"'+(status==='Đã tuyển đủ'?' selected':'')+'>Đã tuyển đủ</option></select> ';
...
function renderRecruitmentDetail(code){var r=recruitmentRequests.find(function(x){return x.code===code});if(!r)return;var c=document.getElementById('recruitmentDetailContent');var h='<div class="pdf-preview"><h2>CHI TIẾT NHU CẦU TUYỂN DỤNG</h2>';
...
h+='<td><select class="status-select cand-status-change" data-code="'+c2.code+'"><option value="Đã cập nhật thông tin"'+(status==='Đã cập nhật thông tin'?' selected':'')+'>Đã cập nhật thông tin</option><option value="Đã hẹn phỏng vấn"'+(status==='Đã hẹn phỏng vấn'?' selected':'')+'>Đã hẹn phỏng vấn</option><option value="Đã xác nhận phỏng vấn"'+(status==='Đã xác nhận phỏng vấn'?' selected':'')+'>Đã xác nhận phỏng vấn</option><option value="Đạt"'+(status==='Đạt'?' selected':'')+'>Đạt</option><option value="Không đạt"'+(status==='Không đạt'?' selected':'')+'>Không đạt</option><option value="Đã xác nhận nhận việc"'+(status==='Đã xác nhận nhận việc'?' selected':'')+'>Đã xác nhận nhận việc</option></select> ';
...
h+='<tr><td>'+(i+1)+'</td><td>'+iv.code+'</td><td>'+iv.candidateCode+'</td><td>'+(cd?cd.fullName:'')+'</td><td>'+iv.position+'</td><td>'+formatDate(iv.date)+'</td><td>'+na(iv.time)+'</td><td>'+na(iv.format)+'</td><td>'+na(iv.interviewer)+'</td><td>'+na(iv.location)+'</td><td>'+getStatusBadge(status)+'</td><td>'+getEditCountBadge(iv)+'</td><td>'+operatorInfo(iv)+'</td><td>'+formatDateTime(iv.timestamp)+'</td>';
h+='<td><select class="status-select iv-status-change" data-code="'+iv.code+'"><option value="Đã lên lịch"'+(status==='Đã lên lịch'?' selected':'')+'>Đã lên lịch</option><option value="Đã phỏng vấn"'+(status==='Đã phỏng vấn'?' selected':'')+'>Đã phỏng vấn</option><option value="Hủy phỏng vấn"'+(status==='Hủy phỏng vấn'?' selected':'')+'>Hủy phỏng vấn</option></select> ';
...
var h='<table id="resultDataTable"><thead><tr><th>STT</th><th>Mã kết quả</th><th>Mã ứng viên</th><th>Họ tên</th><th>Vị trí</th><th>Người phỏng vấn</th><th>Điểm đánh giá</th><th>Kết quả</th><th>Mức lương đề xuất</th><th>Ngày cập nhật</th><th>Ghi chú</th><th>Số lần chỉnh sửa</th><th>Người thao tác</th><th>Thời gian thao tác</th><th>Thao tác</th></tr></thead><tbody>';
...
h+='<tr><td>'+(i+1)+'</td><td>'+r.code+'</td><td>'+r.candidateCode+'</td><td>'+(cd?cd.fullName:'')+'</td><td>'+na(r.position)+'</td><td>'+na(r.interviewer)+'</td><td>'+na(r.score)+'</td><td>'+getStatusBadge(conclusion)+'</td><td>'+na(r.proposedSalary)+'</td><td>'+formatDate(r.updatedDate||r.timestamp)+'</td><td>'+na(r.note)+'</td><td>'+getEditCountBadge(r)+'</td><td>'+operatorInfo(r)+'</td><td>'+formatDateTime(r.timestamp)+'</td>';
h+='<td><select class="status-select res-status-change" data-code="'+r.code+'"><option value="Đạt"'+(conclusion==='Đạt'?' selected':'')+'>Đạt</option><option value="Không đạt"'+(conclusion==='Không đạt'?' selected':'')+'>Không đạt</option><option value="Chờ quyết định"'+(conclusion==='Chờ quyết định'?' selected':'')+'>Chờ quyết định</option></select> ';
...
h+='<tr><td>'+(i+1)+'</td><td>'+ob.code+'</td><td>'+ob.fullName+'</td><td>'+na(ob.department)+'</td><td>'+na(ob.position)+'</td><td>'+formatDate(ob.startDate)+'</td><td>'+na(ob.salary)+'</td><td>'+na(ob.manager)+'</td><td>'+na(ob.contractType)+'</td><td>'+getStatusBadge(status)+'</td><td>'+getEditCountBadge(ob)+'</td><td>'+operatorInfo(ob)+'</td><td>'+formatDateTime(ob.timestamp)+'</td>';
h+='<td><select class="status-select ob-status-change" data-code="'+ob.code+'"><option value="Đang thử việc"'+(status==='Đang thử việc'?' selected':'')+'>Đang thử việc</option><option value="Chính thức"'+(status==='Chính thức'?' selected':'')+'>Chính thức</option><option value="Nghỉ việc"'+(status==='Nghỉ việc'?' selected':'')+'>Nghỉ việc</option></select> ';
...
document.getElementById('btnAddInterview').addEventListener('click',function(){editingInterviewCode=null;document.getElementById('interviewFormTitle').textContent='Tạo lịch phỏng vấn';document.getElementById('ivCandCode').value='';document.getElementById('ivPosition').value='';document.getElementById('ivDate').value='';document.getElementById('ivTime').value='';document.getElementById('ivFormat').selectedIndex=0;document.getElementById('ivInterviewer').value='';document.getElementById('ivLocation').value='';showView('interviewFormView')});

document.getElementById('btnAddResult').addEventListener('click',function(){editingResultCode=null;document.getElementById('resultFormTitle').textContent='Thêm kết quả phỏng vấn';document.getElementById('resCandCode').value='';document.getElementById('resPosition').value='';document.getElementById('resInterviewer').value='';document.getElementById('resScore').value='';document.getElementById('resConclusion').selectedIndex=0;document.getElementById('resSalary').value='';document.getElementById('resNote').value='';showView('resultFormView')});
...
var stamp=getUserStamp();
if(editingResultCode){var r=interviewResults.find(function(x){return x.code===editingResultCode});if(!r)return;if(!r.editHistory)r.editHistory=[];r.editHistory.push({employeeId:stamp.employeeId,employeeName:stamp.employeeName,timestamp:stamp.timestamp,changes:'Cập nhật kết quả'});r.candidateCode=document.getElementById('resCandCode').value.trim();r.position=document.getElementById('resPosition').value.trim();r.interviewer=document.getElementById('resInterviewer').value.trim();r.score=document.getElementById('resScore').value;r.conclusion=document.getElementById('resConclusion').value;r.proposedSalary=document.getElementById('resSalary').value.trim();r.note=document.getElementById('resNote').value.trim();r.updatedDate=getNow();r.employeeId=stamp.employeeId;r.employeeName=stamp.employeeName;r.timestamp=stamp.timestamp;addHistory('Sửa','Kết quả phỏng vấn',editingResultCode,'Đã cập nhật');alert('Cập nhật thành công!');editingResultCode=null}
else{var r={code:generateResultCode(),candidateCode:document.getElementById('resCandCode').value.trim(),position:document.getElementById('resPosition').value.trim(),interviewer:document.getElementById('resInterviewer').value.trim(),score:document.getElementById('resScore').value,conclusion:document.getElementById('resConclusion').value,proposedSalary:document.getElementById('resSalary').value.trim(),note:document.getElementById('resNote').value.trim(),updatedDate:getNow(),employeeId:stamp.employeeId,employeeName:stamp.employeeName,employeePosition:stamp.employeePosition,employeeDept:stamp.employeeDept,timestamp:stamp.timestamp,editHistory:[]};interviewResults.push(r);addHistory('Tạo mới','Kết quả phỏng vấn',r.code,'Đánh giá ứng viên '+r.candidateCode);alert('Lưu kết quả thành công! Mã: '+r.code)}
...
document.getElementById('btnSearchCandidate').addEventListener('click',function(){var f=document.getElementById('candidateSearchFrom').value,t=document.getElementById('candidateSearchTo').value;var txt=(document.getElementById('candidateSearchText').value||'').trim().toLowerCase();var st=document.getElementById('candidateSearchStatus').value;renderCandidateTable(candidates.filter(function(c){var ts=(c.submitDate||c.timestamp||'').substring(0,10);var matchDate=(!f||ts>=f)&&(!t||ts<=t);var matchText=!txt||c.fullName.toLowerCase().includes(txt)||(c.phone&&c.phone.includes(txt))||c.code.toLowerCase().includes(txt);var matchStatus=!st||(c.status||'Đã cập nhật thông tin')===st;return matchDate&&matchText&&matchStatus}))});
