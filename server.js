...
<div><label>Tìm tên/SĐT:</label><br><input type="text" id="recruitSearchText" placeholder="Nhập tên hoặc SĐT"></div>
...
<div><label>Tìm tên/SĐT:</label><br><input type="text" id="candidateSearchText" placeholder="Nhập tên hoặc SĐT"></div>
...
<div><label>Tìm tên/SĐT:</label><br><input type="text" id="interviewSearchText" placeholder="Nhập tên hoặc SĐT"></div>
...
<div><label>Tìm tên/SĐT:</label><br><input type="text" id="resultSearchText" placeholder="Nhập tên hoặc SĐT"></div>
...
<div><label>Tìm tên/SĐT:</label><br><input type="text" id="onboardSearchText" placeholder="Nhập tên hoặc SĐT"></div>
...
function showEvaluationForm(ivCode){var iv=interviews.find(function(x){return
x.code===ivCode});if(!iv)return;var cd=candidates.find(function(c){return
c.code===iv.candidateCode});var
ct=document.getElementById('resultFormContainer');ct.style.display='block';var tc=['Kiến thức chuyên môn','Kinh nghiệm thực tế','Giải quyết vấn đề','Tư duy logic','Kỹ năng công cụ'];var sc=['Giao tiếp','Làm việc nhóm','Chủ động','Khả năng học hỏi','Phù hợp văn hóa'];var h='<div class="form-section" id="evaluationFormInner" data-ivcode="'+ivCode+'"><h3>Đánh giá: '+(cd?cd.fullName:'')+' ('+iv.code+')</h3>';
h+='<div class="form-group"><label>Điểm bài test (nếu có)</label><input type="number" id="evalTestScore" class="score-input" min="0" max="100" step="1"></div>';
h+='<h3>I. Chuyên môn (25 điểm)</h3><table class="score-table"><tbody>';tc.forEach(function(c){h+='<tr><td>'+c+'</td><td><input type="number" class="score-input tech-score" min="0" max="5" step="0.5" value="0"></td></tr>'});h+='</tbody></table>';
h+='<h3>II. Kỹ năng & thái độ (25 điểm)</h3><table class="score-table"><tbody>';sc.forEach(function(c){h+='<tr><td>'+c+'</td><td><input type="number" class="score-input soft-score" min="0" max="5" step="0.5" value="0"></td></tr>'});h+='</tbody></table>';
h+='<p>Điểm tổng: <strong id="totalScoreDisplay">0</strong>/50</p>';
h+='<div class="form-group"><label>Điểm mạnh</label><textarea id="evalStrengths"></textarea></div>';
h+='<div class="form-group"><label>Điểm yếu</label><textarea id="evalWeaknesses"></textarea></div>';
h+='<div class="form-row"><div class="form-group"><label>Vị trí đề xuất</label><input type="text" id="evalProposedPosition" value="'+iv.position+'"></div><div class="form-group"><label>Cấp bậc đề xuất</label><select id="evalProposedLevel"></select></div></div>';
h+='<div class="form-group"><label>Bộ phận đề xuất</label><select id="evalProposedDepartment"></select></div>';
...
populateSelect('evalProposedLevel',levels,'Chọn cấp bậc');
populateSelect('evalProposedDepartment',departments,'Chọn bộ phận');
...
document.getElementById('btnSaveEval').addEventListener('click',function(){var fe=document.getElementById('evaluationFormInner');if(!validateInterviewResultForm(fe))return;var ts=[],ss=[];fe.querySelectorAll('.tech-score').forEach(function(s){ts.push(parseFloat(s.value))});fe.querySelectorAll('.soft-score').forEach(function(s){ss.push(parseFloat(s.value))});var total=0;ts.forEach(function(s){total+=s});ss.forEach(function(s){total+=s});var stamp=getUserStamp();var resultData={interviewCode:ivCode,candidateCode:iv.candidateCode,position:iv.position,techScores:ts,softScores:ss,totalScore:total,suitability:fe.querySelector('input[name="resultSuitability"]:checked').value,conclusion:fe.querySelector('input[name="resultConclusion"]:checked').value,employeeld:stamp.employeeld,employeeName:stamp.employeeName,employeePosition:stamp.employeePosition,employeeDept:stamp.employeeDept,timestamp:stamp.timestamp,expiredStatus:null,editHistory:[],
// Dữ liệu mới
testScore:document.getElementById('evalTestScore').value,
strengths:document.getElementById('evalStrengths').value.trim(),
weaknesses:document.getElementById('evalWeaknesses').value.trim(),
proposedPosition:document.getElementById('evalProposedPosition').value.trim(),
proposedLevel:document.getElementById('evalProposedLevel').value,
proposedDepartment:document.getElementById('evalProposedDepartment').value};
var existingResultIndex=interviewResults.findIndex(function(r){return r.interviewCode===ivCode});
if(existingResultIndex!==-1){interviewResults[existingResultIndex]=resultData}else{interviewResults.push(resultData)}
addHistory('Tạo mới','Đánh giá phỏng vấn',ivCode,'Đánh giá ứng viên '+iv.candidateCode);if(resultData.conclusion==='Đề xuất tuyển'){alert('Ứng viên ĐẠT! Vui lòng điền form thông báo trúng tuyển.');ct.style.display='none';showOfferForm(ivCode)}else{alert('Lưu đánh giá thành công!');ct.style.display='none';renderResultInterviewTable()}})} class="form-section" id="evaluationFormInner" data-ivcode="'+ivCode+'"><h3>Đánh giá: '+(cd?cd.fullName:'')+' ('+iv.code+')</h3><h3>I. Chuyên môn (25 điểm)</h3><table class="score-table"><tbody>';tc.forEach(function(c){h+='<tr><td>'+c+'</td><td><input type="number" class="score-input tech-score" min="0" max="5" step="0.5" value="0"></td></tr>'});h+='</tbody></table><h3>II. Kỹ năng & thái độ (25 điểm)</h3><table class="score-table"><tbody>';sc.forEach(function(c){h+='<tr><td>'+c+'</td><td><input type="number" class="score-input soft-score" min="0" max="5" step="0.5" value="0"></td></tr>'});h+='</tbody></table><p>Điểm tổng: <strong id="totalScoreDisplay">0</strong>/50</p><div class="form-group"><label>Mức độ phù hợp *</label><div class="checkbox-group">';['Rất phù hợp','Phù hợp','Cần cân nhắc','Không phù hợp'].forEach(function(v){h+='<label><input type="radio" name="resultSuitability" value="'+v+'"> '+v+'</label>'});h+='</div></div><h3>Kết luận *</h3><div class="checkbox-group">';['Đề xuất tuyển','Dự bị','Không tuyển'].forEach(function(v){h+='<label><input type="radio" name="resultConclusion" value="'+v+'"> '+v+'</label>'});h+='</div><br><button class="btn btn-success" id="btnSaveEval" style="width:100%;min-height:45px;font-size:16px">Lưu đánh giá</button></div>';ct.innerHTML=h;ct.querySelectorAll('.score-input').forEach(function(inp){inp.addEventListener('input',function(){var t=0;ct.querySelectorAll('.score-input').forEach(function(s){t+=parseFloat(s.value)||0});document.getElementById('totalScoreDisplay').textContent=t})});document.getElementById('btnSaveEval').addEventListener('click',function(){var fe=document.getElementById('evaluationFormInner');if(!validateInterviewResultForm(fe))return;var ts=[],ss=[];fe.querySelectorAll('.tech-score').forEach(function(s){ts.push(parseFloat(s.value))});fe.querySelectorAll('.soft-score').forEach(function(s){ss.push(parseFloat(s.value))});var total=0;ts.forEach(function(s){total+=s});ss.forEach(function(s){total+=s});var stamp=getUserStamp();interviewResults.push({interviewCode:ivCode,candidateCode:iv.candidateCode,position:iv.position,techScores:ts,softScores:ss,totalScore:total,suitability:fe.querySelector('input[name="resultSuitability"]:checked').value,conclusion:fe.querySelector('input[name="resultConclusion"]:checked').value,employeeId:stamp.employeeId,employeeName:stamp.employeeName,employeePosition:stamp.employeePosition,employeeDept:stamp.employeeDept,timestamp:stamp.timestamp,expiredStatus:null,editHistory:[]});addHistory('Tạo mới','Đánh giá phỏng vấn',ivCode,'Đánh giá ứng viên '+iv.candidateCode);alert('Lưu đánh giá thành công!');ct.style.display='none';renderResultInterviewTable()})}
function showResultDetailView(ivCode){var r=interviewResults.find(function(x){return
x.interviewCode===ivCode});if(!r)return;var cd=candidates.find(function(c){return
c.code===r.candidateCode});var ct=document.getElementById('resultDetailContent');var h='<div class="pdf-preview"><h2>PHIẾU ĐÁNH GIÁ ỨNG VIÊN</h2>';
h+='<div class="info-row"><span class="info-label">Mã phỏng vấn:</span><span class="info-value">'+r.interviewCode+'</span></div>';
h+='<div class="info-row"><span class="info-label">Ứng viên:</span><span class="info-value">'+(cd?cd.fullName:'')+' ('+r.candidateCode+')</span></div>';
h+='<div class="info-row"><span class="info-label">Vị trí phỏng vấn:</span><span class="info-value">'+r.position+'</span></div>';
h+='<hr style="margin:10px 0;">';
h+='<div class="info-row"><span class="info-label">Điểm bài test:</span><span class="info-value">'+na(r.testScore)+'</span></div>';
h+='<div class="info-row"><span class="info-label">Điểm chuyên môn:</span><span class="info-value">'+r.techScores.reduce(function(a,b){return a+b},0)+'/25</span></div>';
h+='<div class="info-row"><span class="info-label">Điểm kỹ năng:</span><span class="info-value">'+r.softScores.reduce(function(a,b){return a+b},0)+'/25</span></div>';
h+='<div class="info-row"><span class="info-label">Điểm tổng:</span><span class="info-value"><strong>'+r.totalScore+'/50</strong></span></div>';
h+='<div class="info-row"><span class="info-label">Điểm mạnh:</span><span class="info-value">'+na(r.strengths)+'</span></div>';
h+='<div class="info-row"><span class="info-label">Điểm yếu:</span><span class="info-value">'+na(r.weaknesses)+'</span></div>';
h+='<hr style="margin:10px 0;">';
h+='<div class="info-row"><span class="info-label">Vị trí đề xuất:</span><span class="info-value">'+na(r.proposedPosition)+'</span></div>';
h+='<div class="info-row"><span class="info-label">Cấp bậc đề xuất:</span><span class="info-value">'+na(r.proposedLevel)+'</span></div>';
h+='<div class="info-row"><span class="info-label">Bộ phận đề xuất:</span><span class="info-value">'+na(r.proposedDepartment)+'</span></div>';
h+='<div class="info-row"><span class="info-label">Mức độ phù hợp:</span><span class="info-value">'+r.suitability+'</span></div>';
h+='<div class="info-row"><span class="info-label">Kết luận:</span><span class="info-value"><strong>'+r.conclusion+'</strong></span></div>';
h+='<div class="info-row"><span class="info-label">Người thao tác:</span><span class="info-value">'+operatorFull(r)+'</span></div>';
h+='<div class="info-row"><span class="info-label">Thời gian:</span><span class="info-value">'+formatDateTime(r.timestamp)+'</span></div>';h+=renderEditHistoryHTML(r);h+='</div>';
ct.innerHTML=h;ct.setAttribute('data-ivcode',ivCode);showView('resultDetailView')};h+='<div class="info-row"><span class="info-label">Mã phỏng vấn:</span><span class="info-value">'+r.interviewCode+'</span></div>';h+='<div class="info-row"><span class="info-label">Ứng viên:</span><span class="info-value">'+(cd?cd.fullName:'')+' ('+r.candidateCode+')</span></div>';h+='<div class="info-row"><span class="info-label">Điểm tổng:</span><span class="info-value"><strong>'+r.totalScore+'/50</strong></span></div>';h+='<div class="info-row"><span class="info-label">Mức độ phù hợp:</span><span class="info-value">'+r.suitability+'</span></div>';h+='<div class="info-row"><span class="info-label">Kết luận:</span><span class="info-value"><strong>'+r.conclusion+'</strong></span></div>';h+='<div class="info-row"><span class="info-label">Người thao tác:</span><span class="info-value">'+operatorFull(r)+'</span></div>';h+='<div class="info-row"><span class="info-label">Thời gian:</span><span class="info-value">'+formatDateTime(r.timestamp)+'</span></div>';h+=renderEditHistoryHTML(r);h+='</div>';ct.innerHTML=h;ct.setAttribute('data-ivcode',ivCode);showView('resultDetailView')}
function showOfferForm(ivCode){var rs=interviewResults.find(function(r){return
r.interviewCode===ivCode});if(!rs)return;var cd=candidates.find(function(c){return
c.code===rs.candidateCode});if(!cd)return;var
ct=document.getElementById('offerFormContent');var h='<div class="form-section" id="offerFormInner" data-candcode="'+cd.code+'"><h3>Thông báo trúng tuyển: '+cd.fullName+'</h3>';
h+='<div class="form-row"><div class="form-group"><label>Vị trí trúng tuyển</label><input type="text" id="offerPosition" value="'+(rs.proposedPosition || rs.position)+'"></div><div class="form-group"><label>Cấp bậc</label><select id="offerLevel"></select></div></div>';
h+='<div class="form-group"><label>Bộ phận</label><select id="offerDepartment"></select></div>';
h+='<div class="form-group"><label>Địa điểm làm việc</label><select id="offerWorkplace"><option value="">--Chọn--</option><option>Nhà máy 1</option><option>Nhà máy 2</option><option>Nhà máy 3</option><option>Nhà máy 4</option></select></div>';
h+='<div class="form-group"><label>Ca làm việc</label><select id="offerWorkshift"><option value="">--Chọn--</option><option>Hành chính</option><option>Hành chính kíp</option><option>2 ca</option><option>3 ca</option><option>3 ca kíp</option></select></div>';
h+='<div class="form-group"><label>Ngày nhận việc *</label><input type="date" id="offerStartDate"></div>';
h+='<div class="form-group"><label>Tuyến xe buýt</label><input type="text" id="offerBusRoute" placeholder="Nếu có"></div>';
h+='<div class="form-row"><div class="form-group"><label>Lương thử việc *</label><input type="text" id="offerProbSalary"></div><div class="form-group"><label>Lương chính thức *</label><input type="text" id="offerOfficialSalary"></div></div>';
h+='<div class="form-group"><label>Phụ cấp</label><div class="checkbox-group">';['Ăn ca','Chuyên cần','Nhà ở','Khác'].forEach(function(v){h+='<label><input type="checkbox" name="offerAllowance" value="'+v+'"> '+v+'</label>'});h+='</div></div>';
h+='<div class="form-group"><label>Hình thức trả lương *</label><div class="checkbox-group"><label><input type="radio" name="offerPayMethod" value="Chuyển khoản"> Chuyển khoản</label><label><input type="radio" name="offerPayMethod" value="Tiền mặt"> Tiền mặt</label></div></div>';
h+='<div class="form-group"><label>Hồ sơ cần chuẩn bị</label><div class="checkbox-group">';['Căn cước công dân','Sơ yếu lý lịch','Giấy khám sức khỏe','Bằng cấp','Giấy khai sinh'].forEach(function(v){h+='<label><input type="checkbox" name="offerDocs" value="'+v+'" checked> '+v+'</label>'});h+='</div></div>';
h+='<br><button class="btn btn-success" id="btnSaveOffer" style="width:100%;min-height:45px;font-size:16px">Lưu và Chuyển sang nhận việc</button></div>';
ct.innerHTML=h;
populateSelect('offerLevel',levels,'Chọn cấp bậc',rs.proposedLevel);
populateSelect('offerDepartment',departments,'Chọn bộ phận',rs.proposedDepartment);
if(rs.proposedLevel)setSelectValue('offerLevel',rs.proposedLevel);
if(rs.proposedDepartment)setSelectValue('offerDepartment',rs.proposedDepartment);
showView('offerFormView');document.getElementById('btnSaveOffer').addEventListener('click',function(){var offerForm=document.getElementById('offerFormInner');var requiredFields=['offerStartDate','offerProbSalary','offerOfficialSalary'];var errors=[];requiredFields.forEach(function(id){if(!document.getElementById(id).value.trim())errors.push(document.querySelector('label[for="'+id+'"]').textContent||id)});if(!offerForm.querySelector('input[name="offerPayMethod"]:checked'))errors.push('Hình thức trả lương');if(errors.length>0){alert('Vui lòng điền các trường bắt buộc:\\n- '+errors.join('\\n- '));return}var stamp=getUserStamp();cd.offer={
// Dữ liệu mới
position:document.getElementById('offerPosition').value.trim(),
level:document.getElementById('offerLevel').value,
department:document.getElementById('offerDepartment').value,
workplace:document.getElementById('offerWorkplace').value,
workshift:document.getElementById('offerWorkshift').value,
startDate:document.getElementById('offerStartDate').value,
busRoute:document.getElementById('offerBusRoute').value.trim(),
docs:getCheckedValues('offerDocs'),
// Dữ liệu cũ
probSalary:document.getElementById('offerProbSalary').value.trim(),officialSalary:document.getElementById('offerOfficialSalary').value.trim(),allowances:getCheckedValues('offerAllowance'),payMethod:offerForm.querySelector('input[name="offerPayMethod"]:checked').value,employeeld:stamp.employeeld,employeeName:stamp.employeeName,employeePosition:stamp.employeePosition,employeeDept:stamp.employeeDept,timestamp:stamp.timestamp};addHistory('Tạo mới','Thông báo trúng tuyển',cd.code,'Trúng tuyển: '+cd.fullName);alert('Lưu thông báo trúng tuyển thành công! Chuyển sang xác nhận nhận việc.');renderOnboardingList();showView('onboardingFormView')})}<div class="form-row"><div class="form-group"><label>Lương thử việc *</label><input type="text" id="offerProbSalary"></div><div class="form-group"><label>Lương chính thức *</label><input type="text" id="offerOfficialSalary"></div></div><div class="form-group"><label>Phụ cấp</label><div class="checkbox-group">';['Ăn ca','Chuyên cần','Nhà ở','Khác'].forEach(function(v){h+='<label><input type="checkbox" name="offerAllowance" value="'+v+'"> '+v+'</label>'});h+='</div></div><div class="form-group"><label>Lương đóng bảo hiểm</label><input type="text" id="offerInsuranceSalary"></div><div class="form-group"><label>Hình thức trả lương *</label><div class="checkbox-group"><label><input type="radio" name="offerPayMethod" value="Chuyển khoản"> Chuyển khoản</label><label><input type="radio" name="offerPayMethod" value="Tiền mặt"> Tiền mặt</label></div></div><br><button class="btn btn-success" id="btnSaveOffer" style="width:100%;min-height:45px;font-size:16px">Lưu</button></div>';ct.innerHTML=h;showView('offerFormView');document.getElementById('btnSaveOffer').addEventListener('click',function(){if(!document.getElementById('offerProbSalary').value.trim()){alert('Nhập lương thử việc');return}if(!document.getElementById('offerOfficialSalary').value.trim()){alert('Nhập lương chính thức');return}var pm=document.getElementById('offerFormInner').querySelector('input[name="offerPayMethod"]:checked');if(!pm){alert('Chọn hình thức trả lương');return}var stamp=getUserStamp();cd.offer={probSalary:document.getElementById('offerProbSalary').value.trim(),officialSalary:document.getElementById('offerOfficialSalary').value.trim(),allowances:getCheckedValues('offerAllowance'),insuranceSalary:document.getElementById('offerInsuranceSalary').value.trim(),payMethod:pm.value,employeeId:stamp.employeeId,employeeName:stamp.employeeName,employeePosition:stamp.employeePosition,employeeDept:stamp.employeeDept,timestamp:stamp.timestamp};addHistory('Tạo mới','Thông báo trúng tuyển',cd.code,'Trúng tuyển: '+cd.fullName);alert('Lưu thành công!');renderProposedExcelTable();showView('proposedExcelView')})}
...
document.getElementById('btnSearchRecruitment').addEventListener('click',function(){var f=document.getElementById('recruitSearchFrom').value,t=document.getElementById('recruitSearchTo').value;var txt=(document.getElementById('recruitSearchText').value||'').trim().toLowerCase();renderRecruitmentTable(recruitmentRequests.filter(function(r){var ts=r.timestamp.substring(0,10);var matchDate=(!f||ts>=f)&&(!t||ts<=t);var matchText=!txt||r.proposer.toLowerCase().indexOf(txt)!==-1||r.position.toLowerCase().indexOf(txt)!==-1||r.department.toLowerCase().indexOf(txt)!==-1||r.code.toLowerCase().indexOf(txt)!==-1;return matchDate&&matchText}))});
document.getElementById('btnSearchCandidate').addEventListener('click',function(){var f=document.getElementById('candidateSearchFrom').value,t=document.getElementById('candidateSearchTo').value;var txt=(document.getElementById('candidateSearchText').value||'').trim().toLowerCase();renderCandidateTable(candidates.filter(function(c){var ts=c.timestamp.substring(0,10);var matchDate=(!f||ts>=f)&&(!t||ts<=t);var matchText=!txt||c.fullName.toLowerCase().indexOf(txt)!==-1||c.phone.indexOf(txt)!==-1||c.code.toLowerCase().indexOf(txt)!==-1;return matchDate&&matchText}))});
document.getElementById('btnSearchInterview').addEventListener('click',function(){var f=document.getElementById('interviewSearchFrom').value,t=document.getElementById('interviewSearchTo').value;var txt=(document.getElementById('interviewSearchText').value||'').trim().toLowerCase();renderCandidateTableForInterview(candidates.filter(function(c){var ts=c.timestamp.substring(0,10);var matchDate=(!f||ts>=f)&&(!t||ts<=t);var matchText=!txt||c.fullName.toLowerCase().indexOf(txt)!==-1||c.phone.indexOf(txt)!==-1||c.code.toLowerCase().indexOf(txt)!==-1;return matchDate&&matchText}))});
document.getElementById('btnSearchResult').addEventListener('click',function(){var f=document.getElementById('resultSearchFrom').value,t=document.getElementById('resultSearchTo').value;var txt=(document.getElementById('resultSearchText').value||'').trim().toLowerCase();renderResultInterviewTable(interviews.filter(function(iv){var ts=iv.timestamp.substring(0,10);var matchDate=(!f||ts>=f)&&(!t||ts<=t);var cd=candidates.find(function(c){return c.code===iv.candidateCode});var matchText=!txt||(cd&&cd.fullName.toLowerCase().indexOf(txt)!==-1)||(cd&&cd.phone.indexOf(txt)!==-1)||iv.candidateCode.toLowerCase().indexOf(txt)!==-1;return matchDate&&matchText}))});
document.getElementById('btnSearchOnboarding').addEventListener('click',function(){var f=document.getElementById('onboardSearchFrom').value,t=document.getElementById('onboardSearchTo').value;var txt=(document.getElementById('onboardSearchText').value||'').trim().toLowerCase();renderOnboardingList(interviewResults.filter(function(r){if(r.conclusion!=='Đề xuất tuyển')return false;var ts=r.timestamp.substring(0,10);var matchDate=(!f||ts>=f)&&(!t||ts<=t);var cd=candidates.find(function(c){return c.code===r.candidateCode});var matchText=!txt||(cd&&cd.fullName.toLowerCase().indexOf(txt)!==-1)||(cd&&cd.phone&&cd.phone.indexOf(txt)!==-1)||r.candidateCode.toLowerCase().indexOf(txt)!==-1;return matchDate&&matchText}))});
